...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = 1 - next_h**2
    derivative = dnext_h * dtanh
    db = derivative.sum(axis=0)
    dx = derivative.dot(Wx.T)
    dprev_h = derivative.dot(Wh.T)
    dWx = x.T.dot(derivative)
    dWh = prev_h.T.dot(derivative)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        next_h, next_cache = rnn_step_forward(x[:, t], h0, Wx, Wh, b)
        h0 = next_h
        h.append(next_h)
...
def rnn_backward(dh, cache):
    ...
    dx_cur, dh0_prev, dWx, dWh, db = rnn_step_backward(dh[:, -1], cache[-1])
    dh0 = dh0_prev.copy()
    dx = [dx_cur]
    for t in range(T - 2, -1, -1):
        dh_cur = dh[:, t] + dh0_prev
        dx_cur, dh0_cur, dWx_cur, dWh_cur, db_cur = \
            rnn_step_backward(dh_cur, cache[t])
        dh0 += dh0_cur - dh0_prev
        dWx += dWx_cur
        dWh += dWh_cur
        db += db_cur
        dx.append(dx_cur)
        dh0_prev = dh0_cur
    dx = np.array(dx).transpose(1, 0, 2)[:, ::-1, :]
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        next_h, next_c, next_cache = lstm_step_forward(
...
# dx, dprev_h, dprev_c, dWx, dWh, db
    dc = np.zeros_like(dh[:, -1])
    dx_cur, dh0_prev, dprev_c, dWx, dWh, db = lstm_step_backward(
        dh[:, -1], dc, cache[-1])
    dh0 = dh0_prev.copy()
    dx = [dx_cur]
    for t in range(T - 2, -1, -1):
        dh_cur = dh[:, t] + dh0_prev
        dx_cur, dh0_cur, dc_cur, dWx_cur, dWh_cur, db_cur = \
            lstm_step_backward(dh_cur, dprev_c, cache[t])
        dh0 += dh0_cur - dh0_prev
        dWx += dWx_cur
        dWh += dWh_cur
        db += db_cur
