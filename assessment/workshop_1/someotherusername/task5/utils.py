...
class SkipGramBTTDataGen:
    """
    Generate data for skip-gram algorithm with batch-transpose trick.
    """
    def __init__(
            self,
            texts: Iterable[List],
    ...
    def iter_line(self) -> Tuple[Any, Any, List[Any]]:
        """
        Draw a set of (*left_context_words, center_word, *right_context_word) 
        from the dataset.

        To make correct batches, we need to start at ws and end ws token from 
        the end.
        """
        for text in self.texts:
            indices = list(range(self.ws, len(text) - self.ws))
            np.random.shuffle(indices)
            for i in indices:  # draw a random word
                context_indices = [i-n for n in range(self.ws, 0, -1)] + [i] + \
                                  [i+1+n for n in range(self.ws)]  # get left, center and right context
                words = [text[i] for i in context_indices]
                yield words
